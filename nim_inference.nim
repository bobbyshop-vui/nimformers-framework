...
import nimpy
import customfloat, quant, backend
...
proc forwardLinear*(l: Linear, x: seq[float32], ctx: Backend): seq[float32] =
  let rows = x.len div l.in_features
  var y = beMatmul(ctx, x, rows, l.in_features, l.weight, l.in_features, l.out_features)
  if l.bias.len > 0:
    for i in 0 ..< y.len:
...
proc forwardMHA*(mha: MultiHeadAttention, x: seq[float32], ctx: Backend,
...
proc forwardSwiGLU*(ff: SwiGLU, x: seq[float32], ctx: Backend): seq[float32] =
...
proc forwardBlock*(blk: TransformerBlock, x: seq[float32], ctx: Backend,
                    B, T, C: int): seq[float32] =
...
proc forwardLlama*(model: LlamaModel, input_ids: seq[int], ctx: Backend): seq[float32] =
  let B = 1
  let T = input_ids.len
...
proc generate*(model: LlamaModel, tokenizer: HFTokenizer, ctx: Backend,
