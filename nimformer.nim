...
proc forward*(ln: LayerNorm, x: Tensor, ctx: MetalContext): Tensor =
  ...
  var result = newTensor(x.shape)
  for r in 0 ..< rows:
    let off = r * cols
    var mean = 0'f32
    for c in 0 ..< cols: mean += x.data[off + c]
    mean /= float32(cols)
    var varr = 0'f32
    for c in 0 ..< cols:
      let d = x.data[off + c] - mean
      varr += d * d
    varr /= float32(cols)
    let inv = 1'f32 / sqrt(varr + ln.eps)
    for c in 0 ..< cols:
...
proc backward*(ln: LayerNorm, x: Tensor, dOut: Tensor, ctx: MetalContext): tuple[dX, dGamma, dBeta: Tensor] =
  ...
  for r in 0 ..< rows:
    ...
    # dGamma, dBeta
    for c in 0 ..< cols:
      let norm = (x.data[off + c] - mean) * inv
      dGamma.data[c] += dOut.data[off + c] * norm
      dBeta.data[c] += dOut.data[off + c]

    # dX
    var sum1 = 0'f32
    var sum2 = 0'f32
    for c in 0 ..< cols:
      let grad = dOut.data[off + c] * ln.gamma.data[c] * inv
      sum1 += grad
      sum2 += grad * (x.data[off + c] - mean)
    sum2 *= (-inv * inv / float32(cols))
    for c in 0 ..< cols:
      let term1 = dOut.data[off + c] * ln.gamma.data[c] * inv
      let term2 = sum1 / float32(cols)
      let term3 = (x.data[off + c] - mean) * sum2 * 2.0 / float32(cols)
      dX.data[off + c] = term1 - term2 + term3
...
proc softmaxInplace(row: var seq[float32]) =
  var mx = row[0]
  for v in row:
    if v > mx: mx = v
  var s = 0'f32
...
proc forward*(attn: CausalSelfAttention, x: Tensor, ctx: MetalContext): Tensor =
  ...
  for b in 0 ..< B:
    let baseQKV = b * T * 3 * C
    let baseOut = b * T * C
    for h in 0 ..< attn.nHeads:
      let offQ = h * hd
      let offK = C + h * hd
      let offV = 2 * C + h * hd
      for ti in 0 ..< T:
        var scores = newSeq[float32](ti + 1)
        for tj in 0 .. ti:
          var s = 0'f32
          for d in 0 ..< hd:
            s += qkv.data[baseQKV + ti * 3 * C + offQ + d] *
                 qkv.data[baseQKV + tj * 3 * C + offK + d]
          scores[tj] = s * scale
        softmaxInplace(scores)
        for d in 0 ..< hd:
          var acc = 0'f32
          for tj in 0 .. ti:
            acc += scores[tj] * qkv.data[baseQKV + tj * 3 * C + offV + d]
...
proc backward*(attn: CausalSelfAttention, x: Tensor, dOut: Tensor, ctx: MetalContext): tuple[
  ...
  # Forward lại qkv và attnOut (cần cho backward, vì proj.backward cần input
  # gốc của proj, tức attnOut trước khi qua proj — giống hệt logic forward()).
  let qkv = attn.qkv.forward(x, ctx)
  let scaleFwd = 1'f32 / sqrt(float32(hd))
  var attnOut = newTensor(@[B, T, C])
  for b in 0 ..< B:
    let baseQKV = b * T * 3 * C
    let baseOut = b * T * C
  ...
  # Backward qua proj (input gốc của proj là attnOut vừa recompute ở trên)
  let dProj = attn.proj.backward(attnOut, dOut, ctx)

  # Backward qua attention (tính dAttnOut)
  let dAttnOut = dProj.dX

  # Backward qua attention core
  # attnOut[ti,h,d] = sum_{tj<=ti} softmax_ti[tj] * V[tj,h,d]
  # với softmax_ti = softmax(scale * Q[ti,h,:] . K[tj,h,:])  (tj <= ti)
  #
  # Gradient (cho từng ti, h cố định):
  #   dV[tj,h,d]      += softmax_ti[tj] * dAttnOut[ti,h,d]                (mọi tj<=ti)
  #   dSoftmax[tj]     = sum_d dAttnOut[ti,h,d] * V[tj,h,d]
  #   dScore[tj]       = softmax_ti[tj] * (dSoftmax[tj] - sum_tk softmax_ti[tk]*dSoftmax[tk])
  #   dScore[tj]      *= scale   (vì score = scale * Q.K)
  #   dQ[ti,h,d]      += dScore[tj] * K[tj,h,d]   (tổng theo tj<=ti)
  #   dK[tj,h,d]      += dScore[tj] * Q[ti,h,d]   (mỗi tj nhận từ mọi ti>=tj)
  ...
  for b in 0 ..< B:
    ...
    for h in 0 ..< nHeads:
      ...
      let offV = 2 * C + h * hd
      for ti in 0 ..< T:
        # Recompute raw scores rồi softmax (PHẢI softmaxInplace, không được
        # coi scores thô là softmax — đây là bug cũ gây sai toàn bộ backward).
        ...
        for tj in 0 .. ti:
          ...
          for d in 0 ..< hd:
            s += qkv.data[baseQKV + ti * 3 * C + offQ + d] *
                 qkv.data[baseQKV + tj * 3 * C + offK + d]
          softmaxW[tj] = s * scale
        softmaxInplace(softmaxW)

        # dV += softmax * dAttnOut, và dSoftmax[tj] = dAttnOut[ti] . V[tj]
        var dSoftmax = newSeq[float32](ti + 1)
        for tj in 0 .. ti:
          var dotVal = 0'f32
          for d in 0 ..< hd:
            let dOutVal = dAttnOut.data[baseOut + ti * C + h * hd + d]
            dQkv.data[baseQKV + tj * 3 * C + offV + d] += softmaxW[tj] * dOutVal
            dotVal += dOutVal * qkv.data[baseQKV + tj * 3 * C + offV + d]
          dSoftmax[tj] = dotVal

        # Softmax backward (Jacobian): dScore_j = y_j * (dy_j - sum_k y_k*dy_k)
        var dotSum = 0'f32
        for tj in 0 .. ti:
        ...
        for tj in 0 .. ti:
          ...
          for d in 0 ..< hd:
            dQkv.data[baseQKV + ti * 3 * C + offQ + d] +=
                dScore * qkv.data[baseQKV + tj * 3 * C + offK + d]
            dQkv.data[baseQKV + tj * 3 * C + offK + d] +=
                dScore * qkv.data[baseQKV + ti * 3 * C + offQ + d]

  # Backward qua qkv
  let dQkvLin = attn.qkv.backward(x, dQkv, ctx)
  let dX = dQkvLin.dX
  
  result = (dX, dQkvLin.dW, dQkvLin.dB, dProj.dW, dProj.dB)

# ===================================================================
# FeedForward (forward/backward dùng Metal matmul)
# ===================================================================
proc apfluAct(x: float32): float32 {.inline.} =
  if x > 0'f32: x * 1.1'f32 else: x * 0.1'f32

proc apfluDeriv(x: float32): float32 {.inline.} =
  if x > 0'f32: 1.1'f32 else: 0.1'f32
...
proc backward*(ff: FeedForward, x: Tensor, dOut: Tensor, ctx: MetalContext): tuple[
  ...
  for i in 0 ..< dHidden.data.len:
    dHidden.data[i] *= apfluDeriv(hidden.data[i])
